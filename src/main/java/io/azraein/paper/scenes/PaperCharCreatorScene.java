...
public class PaperCharCreatorScene extends PaperScene {
	...
	private final String[] defaultNames = { "Montesque", "Jibblets", "Tommy", "Gibby", "Bill", "Ted", };
	...
	public PaperCharCreatorScene(PaperApp paperApp) {
		...
		createCharacterBtn.setOnAction(e -> {
			...
			if (playerName.isEmpty())
				playerName = defaultNames[Paper.rnJesus.nextInt(defaultNames.length)];
			...
			paperApp.playerProperty().get().addItem(Registry.getItem("testHands"), 0);
		...
		characterBackgroundComboBox.getSelectionModel().selectedItemProperty()
				.addListener(((observable, oldValue, newValue) -> {
					...
					if (newValue != null) {
						...
						else if (newValue.equalsIgnoreCase("Soldier"))
							characterBackground = cbf.createSoldierBackground();
